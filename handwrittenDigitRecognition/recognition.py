...
model = tf.keras.models.load_model('model2_filename.keras')
...
while True:
    ...
    # Check if game quit
    # Event Handling:
    # The pygame.event.get() retrieves a list of all events (e.g., key presses, mouse movements, window close requests).
    # The for loop iterates through these events.
    # Quit Event:
    # If the event type is pygame.QUIT (triggered when the user clicks the window's close button), the program exits using sys.exit().
    ...
    # pygame.mouse.get_pressed() returns a tuple of three boolean values:
    # click: State of the left mouse button.
    # _: State of the middle mouse button (unused here).
    # _: State of the right mouse button (unused here).
    # Purpose:
    # Detects if the user is holding down the left mouse button (click == 1).
    ...
    # Reset button
    # Position: 30 pixels from the left and just below the grid (OFFSET + ROWS * CELL_SIZE + 30).
    # Size: 100 pixels wide and 30 pixels high.
    resetButton = pygame.Rect(
        30, OFFSET + ROWS * CELL_SIZE + 30,
        100, 30
    )
    # The button is drawn as a white rectangle with the word "Reset" in the center.
    ...
    classifyText = smallFont.render("Classify", True, GREEN)
    ...
    # Reset Functionality: If the button is clicked, the handwriting grid is reset to all zeros, effectively clearing the screen.
    # The classification is also set to None.
